...
「上一卡 / 下一卡」按你进来的那条路径翻：从先学 20 进来就按这 20 张的顺序（可以跨模块），从主干 42 进来就按 01→42，从全部卡片进来只在当前模块组内前后翻，不跨板块。这对链接常驻在视口右边缘，是一条窄竖条：上一卡在上、下一卡在下，不压正文，宽屏窄屏都能点；手机和小平板竖屏（窄于 980px）正文末尾再补一份带进度和卡片标题的完整版，宽屏有了竖条就不重复放。
...
catalog.js             完整目录数据
card-nav.js            卡片内 prev/next 导航：顺序跟随入口（?p=core20 走先学 20，?p=main42 走主干 42，无参数走全部卡片的组内顺序）；右边缘常驻竖条（宽屏）+ 正文末尾完整版（窄于 980px）
