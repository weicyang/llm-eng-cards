...
391 张卡片 · 36 张主干 · 原理 → 应用 → 生产 · 72 篇高频问答
...
高频问答 72 篇是独立板块（模块 / 全链路 / Agent / RAG / LLM / 工程六组），在首页接在全部卡片之后、最新更新之前，不计入上面的卡片数。每篇是一组问答，不是单个知识点：挑一组当清单过一遍，卡住了再回去翻对应的卡片。
...
```
index.html             学习入口：36 张主干 + 目标入口 + 完整目录 + 高频问答
catalog.js             完整目录数据
qa-catalog.js          高频问答目录数据（独立于 catalog.js）
LEARNING_PATH.md       主干编号与链接
cards/                 按主题存放，便于维护，不是阅读顺序
cards/interview/       高频问答页
