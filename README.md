...
卡片底部的「上一卡 / 下一卡」按你进来的那条路径翻：从先学 20 进来就按这 20 张的顺序（可以跨模块），从主干 42 进来就按 01→42，从全部卡片进来只在当前模块组内前后翻，不跨板块。
...
catalog.js             完整目录数据
card-nav.js            卡片内 prev/next 导航：顺序跟随入口（?p=core20 走先学 20，?p=main42 走主干 42，无参数走全部卡片的组内顺序）
