...
「上一卡 / 下一卡」按你进来的那条路径翻：从先学 20 进来就按这 20 张的顺序（可以跨模块），从主干 42 进来就按 01→42，从全部卡片进来只在当前模块组内前后翻，不跨板块。这对链接常驻在视口右下角，随时可点（窄屏收成两个圆形箭头按钮），正文末尾另有一份带进度的完整版。
...
catalog.js             完整目录数据
card-nav.js            卡片内 prev/next 导航：顺序跟随入口（?p=core20 走先学 20，?p=main42 走主干 42，无参数走全部卡片的组内顺序）；右下角常驻悬浮条 + 正文末尾完整版
